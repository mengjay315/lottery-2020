...
func SignIn(c *gin.Context) () {
	...
	_, err := zcpg.InsertPerson(db, person)
	if err != nil {
	...
	if queryProg.Depart == "" {
		_, err = zcpg.InsertProgram(db, program)
		if err != nil {
...
func SetVoteID(c *gin.Context) () {
	...
	// 更新vote ID
	db := database.InitDB()
	_, err := zcpg.UpdateVoteID(db, &voteID)
	if err != nil {
...
func Vote(c *gin.Context) () {
	...
	if vidRes.Vid == 1 {
		...
		if person.Able == 0 {
			...
			// 更新depart number
			db := database.InitDB()
			_, err = zcpg.UpdateProgram(db, &program)
			if err != nil {
			...
			_, err = zcpg.UpdatePerson(db, &person)
			if err != nil {
